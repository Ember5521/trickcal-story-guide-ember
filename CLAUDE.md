...
**연결선은 그리지 않는다.** PC 는 큐레이션 원(96px)의 중심을 앵커 카드의 위쪽 모서리에 맞춰 원의 1/4 이 카드에 물리게 놓는다. 좌표는 앵커에서 파생되므로 저장된 `x`/`y` 는 무시하고 드래그도 막는다. 물린 큐레이션은 불투명하게 칠한다 — 반투명이면 카드가 비쳐서 모서리가 파인 것처럼 안 읽힌다. 엣지 선은 관리자에게만 남긴다(연결을 지우려면 클릭할 대상이 있어야 한다).

모바일 카드는 70px 한 줄이라 코너에 물릴 자리가 없다. 카드 오른쪽 끝의 **버튼 열**에 시청 체크와 세로로 정렬해 얹는다(큐레이션 위, 체크 아래). 오탭 방지가 목적이므로 둘을 가로로 붙이지 말 것.

노트 창은 PC·모바일 모두 **화면 정중앙**이다. PC 는 `createPortal` 로 `document.body` 에 붙인다 — React Flow 가 뷰포트에 CSS transform 을 걸기 때문에 노드 안에 두면 줌 배율만큼 같이 커지고 화면 밖으로도 나간다.
